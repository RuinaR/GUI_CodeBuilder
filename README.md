...
드래그 앤 드롭 방식으로 GUI를 구성하고,  
공통 IR(`page_ir.json`)을 기준으로 **Flutter / Flet / PyQt6 / HTML/CSS 코드로 export**할 수 있는 GUI 코드 생성기입니다.

단순히 화면만 그리는 툴이 아니라,  
**에디터 상태와 export 대상 코드를 분리**하고,  
하나의 위젯 트리를 여러 플랫폼 코드로 변환하는 구조를 만드는 데 집중했습니다.
...
- 드래그 앤 드롭 기반 GUI 에디터
- 공통 IR(`page_ir.json`) 기반 저장 / 로드 / export
- Flutter / Flet / PyQt6 / HTML/CSS 코드 생성
- 위젯 트리 기반 편집
- Property panel을 통한 위젯 속성 편집
- GitHub Pages를 통한 Web 데모 제공

## Design Notes

이 프로젝트는 다음 흐름을 중심으로 설계했습니다.

- 에디터 내부에서는 위젯 데이터를 트리 상태로 유지
- 저장 시 이를 IR JSON으로 직렬화
- 각 exporter가 같은 IR을 읽어 플랫폼별 코드 생성
- 다시 로드할 때도 generated code가 아니라 IR을 기준으로 에디터 트리를 복원

즉, **IR을 단일 기준 데이터로 두고**,  
에디터와 exporter가 모두 그 구조를 공유하도록 설계했습니다.
